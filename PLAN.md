...
| 11. Audit trail | **Done** | `v0.11.0` | `f4b005a` | 2026-08-20 |
...
### Phase 11 — 2026-08-20 (`v0.11.0`, commit `f4b005a`)

**Design decisions made while implementing this:**

- **A real discovery before any code was written:** `audit_log` itself — the table, its `row_hash`/`prev_row_hash` columns, RLS, and its INSERT-only grant (`SELECT, INSERT` only, no `UPDATE`/`DELETE` for `sentrilog_app`) — was already fully scaffolded back in `001_schema.sql`/`002_multi_tenancy.sql` (Phase 1/2), unusually far ahead of when it was actually needed. No schema migration was required for this phase at all: the entire job was application code that finally writes to a table that had been sitting ready since week one.
- **The hash chain is scoped per tenant, not one global chain.** `record()`'s `prev_row_hash` lookup is `WHERE tenant_id = $1 ORDER BY id DESC LIMIT 1`, not unscoped — each tenant's history is independently verifiable without needing any other tenant's rows, matching the RLS-per-tenant isolation this project already holds everywhere else. The tradeoff: verifying "the whole system" means walking N independent chains, not one; `scripts/verify_audit_chain.py` does exactly that when run with no tenant-id argument.
- **A real concurrency hazard, solved with `pg_advisory_xact_lock`, not client-side retry.** Computing `row_hash(N)` requires reading `row_hash(N-1)` first — a plain read-then-insert, which is a genuine race: two activities processing different cases for the *same* tenant concurrently could both read the same "last" row and each compute a hash chained to it, forking the chain instead of extending it. `record()` takes `pg_advisory_xact_lock(hashtext('audit_log:' || tenant_id))` before reading, serializing writers per-tenant (different tenants never block each other) for exactly the lifetime of that one transaction. Proven, not assumed: `test_concurrent_writes_for_the_same_tenant_never_fork_the_chain` fires 20 real concurrent `record()` calls and asserts one unbroken chain of 20 comes out, not a fork.
- **One canonical hash function, shared across service boundaries, not duplicated.** `services/pipeline/audit.py` (Temporal activities) and `services/intake/audit.py` (case creation, review decisions) both append to the *same* per-tenant chain from two separate processes with two separate DB pools. Everywhere else two services need the same DB-access pattern (e.g. `tenant_connection()`), this project deliberately duplicates it rather than sharing code, to keep services independently deployable — but the `row_hash` *formula* is a correctness invariant the two writers must agree on byte-for-byte, not just a convenience helper, so it lives once in a new `services/common/` package (`audit_hash.py`) that both import, rather than as two copies that could silently drift apart the first time both services wrote to the same tenant's chain back-to-back.
- **Entry/exit pairing via a context manager, not a decorator.** `audit.audited(tenant_id, case_id, step)` writes a `{step}.started` row on entry and a `{step}.completed` or `{step}.failed` row on exit (exception message as payload), yielding a small `AuditExit` object the caller sets `.payload`/`.model_version`/`.input_hash` on before the block ends — since each activity's "what actually happened" summary differs and isn't known until partway through its own work, a decorator wrapping the whole function couldn't capture it as naturally as a `with`-block the caller populates from the inside.
- **A `.started`-with-no-matching-`.completed`/`.failed` row is itself meaningful, not an edge case to eliminate.** It's exactly the signature of a worker crashing mid-activity — the real operational signal Phase 12's "worker died mid-case" runbook (§9) needs to be able to detect. Every activity, including the merely-read-only `fetch_case_documents_activity`, gets the same entry/exit treatment for this reason, not just the ones that mutate state.
- **Scope: Temporal activities plus the two human/API-triggered events that bookend a case's lifecycle — not every intake endpoint.** `audit_log.case_id` is `NOT NULL`, which already rules out tenant-level-but-not-case-scoped actions (API key/reviewer/webhook CRUD) from this table by design — those aren't part of a case's own audit trail. Within what *is* case-scoped, `case_created` and `review_decision_recorded` were added on top of the 8 Temporal activities because a KYC/AML audit trail that couldn't show who approved a case and why would be complete by the letter of "every activity" but useless for the trail's actual regulatory purpose. Claiming a case (advisory-only, not a decision — see Phase 7/9) deliberately still isn't audited; it already gets a real-time `pg_notify` (Phase 10) but no permanent record, consistent with it never having been treated as a state transition worth persisting.
- **An idempotent replay writes no second audit row.** `create_case`'s idempotency-key retry path returns the same case without re-inserting it; auditing it as `case_created` a second time would misrepresent the trail (the case wasn't created twice) rather than complete it.

**Done:**

- `services/common/audit_hash.py`: `compute_row_hash()` (the shared formula, `\x1e`-joined fields — a control-character separator specifically so no field's own content can shift a delimiter and produce a colliding hash for different inputs) and `canonical_payload_json()` (sorted-key JSON so equal payload dicts always hash identically regardless of Python's own key-insertion order).
- `services/pipeline/audit.py`: `record()` (the advisory-lock-guarded single-row writer) and `audited()` (the entry/exit context manager). All 8 Temporal activities retrofitted: `fetch_case_documents`, `extract_document` (`input_hash` = sha256 of the fetched image bytes, `model_version` = the actual Gemini model or `mrz-icao9303`), `face_match` (`input_hash` = sha256 of both photos together), `sanctions_screen`, `update_case_status`, `risk_score`, `finalize_case`, `deliver_webhooks`.
- `services/intake/audit.py`: `record()` (same algorithm, independently implemented per this project's established per-service duplication pattern). `POST /cases` writes `case_created` (skipped on idempotent replay); `POST /review/cases/{id}/decision` writes `review_decision_recorded`, before the workflow-signal attempt so it's recorded even in the already-documented "workflow no longer running" 409 case.
- `scripts/verify_audit_chain.py`: walks one tenant's chain (or every tenant with any rows, if none given) recomputing each row's hash via the shared formula and checking `prev_row_hash` linkage, reporting every violation found; connects via the Postgres superuser role (not `sentrilog_app`'s own DSN), since `audit_log`'s RLS policy would otherwise scope a plain connection to whatever tenant context it never sets — a real bug caught while building this, see below.

**Real bugs found and fixed while implementing this:**

1. **The verification script silently found zero rows for a real tenant with real rows**, reporting "chain OK" — worse than a crash, since it looked like success. Root cause: its first draft connected via `services.intake.config.settings.database_url` (the `sentrilog_app` role, RLS-restricted), the same way every other DB access in this project correctly does — except a tenant-verification tool run out-of-band, with no request-scoped `app.tenant_id` ever set, is precisely the case RLS is designed to show *nothing* to. Fixed by connecting as the Postgres superuser (`sentrilog`, the same role test cleanup helpers already use, matching `002_multi_tenancy.sql`'s own comment that RLS is a no-op for superusers/table owners) — an auditor legitimately needs the access a normal request never should. Caught by an actual CLI run against real seeded data, not by the pytest suite: the unit-style tests for `verify_chain()` passed rows in directly and never exercised the script's own connection logic, so a dedicated regression test (`test_cli_main_finds_real_rows_via_its_own_admin_connection`) was added specifically to close that blind spot.
2. **Every test file that deletes a tenant's data (`tests/intake/conftest.py`, `tests/pipeline/workflows/test_risk_and_finalize_activities.py`, `tests/pipeline/workflows/test_deliver_webhooks_activity.py`) started failing on cleanup** the moment anything actually began writing to `audit_log` — `audit_log.case_id REFERENCES cases(id)` with no `ON DELETE` clause, so deleting a case that now has real audit rows violates the FK. Fixed by deleting `audit_log` rows before `cases` in all three cleanup helpers, the same ordering constraint `claimed_by_reviewer_id` already forces on `cases` vs. `reviewers`.
3. **A CHAR(64)-padding test artifact, not a production bug:** `input_hash`/`row_hash`/`prev_row_hash` are `CHAR(64)` (sized for a real sha256 hexdigest, always exactly 64 characters in every real caller). An early test asserted against a short, non-hash-shaped placeholder string and failed on Postgres's blank-padding of the stored value to the column's fixed width — fixed by using a realistic 64-character value in the test, not by relaxing the column type; every actual caller already only ever passes a real hexdigest.

**Testing evidence (senior-engineer standard, per §9):**

- **21 new tests**, all against real Postgres, none of it mocked: `tests/pipeline/test_audit.py` (16) covers `compute_row_hash` determinism and per-field sensitivity, `record()`'s chain-extension and its concurrency safety (20 real concurrent writers, one unbroken chain), `audited()`'s started/completed and started/failed pairing, the tamper-detection exit criterion itself (write 3 real rows, tamper one via a superuser connection exactly the way an attacker with DB access would have to, confirm `verify_chain()` names the tampered row), and the CLI's own connection logic end-to-end. `tests/pipeline/workflows/test_risk_and_finalize_activities.py` gets 3 new tests confirming `risk_score`/`update_case_status`/`finalize_case` each leave the correct `.started`/`.completed` pair. `tests/intake/test_cases.py` gets 2 (a real `case_created` row, and no duplicate on idempotent replay); `tests/intake/test_review.py`'s existing "decision recorded even with no live workflow" test is extended to also assert the `review_decision_recorded` audit row. Full backend suite: **150 passed** (up from 129).
- **A real, driven CLI verification**, not just the imported function: seeded a real tenant, wrote 2 real rows via `audit.record()`, ran `uv run python scripts/verify_audit_chain.py <tenant-id>` for real and got `chain OK`; then tampered a historical row's `payload` via a direct superuser `UPDATE` (simulating exactly the DB-access-level attacker this feature defends against) and re-ran the same command, which correctly reported the break and exited `1`. This is the literal exit criterion PLAN.md states for this phase.
- **Known gap (explicit, not silent):** the verification script's superuser DSN is the same hardcoded local-dev credential every test file's cleanup helper already uses (`postgresql://sentrilog:sentrilog@localhost:5432/sentrilog`) — fine for this environment, but a real deployment needs this sourced from secrets management with a dedicated, narrowly-scoped read-only auditor role rather than the full table-owner superuser. Out of scope here (Phase 13's AWS/secrets-management territory per §9), but named so it isn't mistaken for a production-ready credential story.
- **Known gap (explicit, not silent):** retryable failures inside `extract_document_activity`/`face_match_activity` (the documented "document not yet uploaded, Temporal will retry" `ApplicationError`) produce a real `.failed` audit row on every retry attempt, not just on a genuine terminal failure — technically accurate (that specific attempt did fail) but means a slow upload can leave several `.failed` rows in the trail before a `.completed` one. Not fixed here: distinguishing "expected retry" from "real failure" in the audit trail would need a new event-type convention not currently justified by any consumer of this data.
